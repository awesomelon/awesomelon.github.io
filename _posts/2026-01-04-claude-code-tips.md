...
성격이 다른 작업(리서치/코딩/검증/문서화)은 세션 단위로 분리합니다. 첫 메시지에 목표와 완료 조건을 적어두면 나중에 다시 들어갔을 때 시간을 아낄 수 있습니다. 세션을 늘리는 것 자체가 목적이 되지 않게 주의하세요. 세션 수가 늘면 관리 비용도 생깁니다.

## 3. Opus 4.5 + thinking을 기본값으로

Boris는 Opus 4.5 + thinking을 거의 모든 작업의 기본값으로 씁니다. Sonnet보다 느릴 수 있지만 덜 조종해도 되고 tool use가 안정적이라 전체 속도가 더 빠른 경우가 많다고 해요.

모델은 응답 한 번의 속도로만 재기 어렵습니다. 작업이 끝날 때까지 몇 번 왕복하느냐로 보는 게 낫습니다. 복잡한 작업은 큰 모델이 유리하고 단순 생성/요약은 작은 모델로도 충분합니다. 팀 예산이 제한적이면 기본은 가볍게 가고 중요한 구간만 올리는 전략이 현실적입니다.
...
프롬프트는 팀이 유지보수하는 규칙 집합입니다. "절대 하면 안 되는 것"과 "PR 전에 반드시 해야 하는 것"을 먼저 고정합니다. Claude가 반복적으로 틀리는 패턴이 보이면 즉시 추가합니다. 단, CLAUDE.md는 회귀 위험이 큰 것 위주로 유지하세요. 너무 길어지면 그만큼 비용이 올라갑니다.
...
계획이 괜찮으면 구현 속도가 빨라지고 리뷰/회귀 위험도 줄어듭니다. 변경 범위, 리스크, 테스트 전략, 롤백이 빠지지 않았는지 확인하세요. 계획이 마음에 들 때까지 질문으로 다듬은 뒤 실행으로 넘어갑니다. 길게 쓰는 게 목적은 아니고 바로 실행할 수 있는 체크리스트면 충분합니다.
...
내 inner loop(커밋/푸시/PR/리포맷/테스트)를 나열하고 빈도가 높은 1~2개부터 slash command로 고정하세요. 인라인 bash는 편하지만 환경 의존이 커질 수 있습니다. 팀 공유 커맨드는 최소한의 가정 위에서 설계하는 게 좋습니다.
...
프로젝트에 맞는 포맷터(prettier/rubocop/black 등)를 훅에서 실행합니다. 훅이 실패해도 진행을 막지 않도록(`|| true`) 설계하되, CI에서는 잡히게 만드는 전략도 있습니다. 자동 수정은 왜 바뀌었는지 안 보일 수 있습니다. 팀이 훅 동작을 이해하도록 문서화하세요.
...
`git status`, 테스트/린트 실행 등 안전한 읽기/검증 커맨드부터 허용합니다. 허용을 넓히는 만큼 위험도 커집니다. 파괴적 명령(삭제/배포/비밀값 출력)은 항상 예외로 두세요. 허용 목록은 코드리뷰를 통해 관리합니다.
...
팀에서 실제로 쓰는 도구(로그/분석/티켓/메신저)부터 연결하고 민감정보와 권한은 최소 권한 원칙으로 분리합니다. 외부 시스템 연동은 보안, 감사, 권한 관리가 중요합니다. 팀 정책에 맞춰 운영하세요.
