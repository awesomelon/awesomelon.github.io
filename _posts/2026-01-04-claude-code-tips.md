...
Boris는 터미널에서 Claude 세션 5개를 동시에 돌리고, 탭을 1~5로 번호 붙여 관리합니다. 포인트는 화면을 계속 지켜보지 않고 알림으로 회수하는 것입니다.
...
Boris는 Opus 4.5 + thinking을 거의 모든 작업의 기본값으로 씁니다. Sonnet보다 느릴 수 있지만, 덜 조종해도 되고 tool use가 안정적이라 결국 전체 속도가 더 빠른 경우가 많다고 해요.

모델은 한 번의 응답 속도보다 작업이 끝날 때까지 몇 번 왕복해야 하는지로 보는 게 낫습니다. 복잡한 작업은 큰 모델이 유리하고, 단순 생성/요약은 작은 모델로도 충분합니다. 팀 예산이 제한적이면 "기본은 가볍게, 중요한 구간만 상향" 전략이 현실적입니다.
...
팀에서 실제로 쓰는 도구(로그/분석/티켓/메신저)부터 연결하고, 민감정보와 권한은 최소 권한 원칙으로 분리합니다. 외부 시스템 연동은 보안, 감사, 권한 관리가 중요하니 팀 정책에 맞춰 운영하세요.
