...
모델 크기를 키울 때 μP(Maximal Update Parametrization)를 사용해 파라미터를 조정하는 것은 이미 알려진 사실입니다. 이 논문은 여기서 더 나아가 배치 크기와 학습 기간이 변할 때도 학습의 안정성을 유지하는 Complete(d) Parameterisation을 제안합니다.
...
이 논문의 메시지는 명확합니다. 하이퍼파라미터 탐색을 작은 모델에서 수행하고, 파라미터화 규칙을 통해 큰 모델로 전이하면, 대규모 학습에서 반복 튜닝 비용을 줄이면서도 안정적인 학습 동역학을 유지할 수 있습니다.
