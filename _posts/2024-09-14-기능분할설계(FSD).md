...
pin: false
description: 큰 프론트엔드 애플리케이션을 나누어 관리하는 FSD 구조를 소개합니다.
---
...
| **processes** | 여러 페이지에 걸친 프로세스 관리 (현재 공식 문서에서는 사용 중단 권고) | 다단계 체크아웃, 온보딩 |
...
이 규칙은 코드의 import 의존 방향을 제한합니다. 런타임 데이터가 한 방향으로만 흐른다는 뜻은 아니에요. 같은 슬라이스 내부 파일은 서로 참조할 수 있지만, 같은 레이어의 다른 슬라이스를 직접 import하는 것은 원칙적으로 금지됩니다. `app`과 `shared`는 슬라이스 없이 세그먼트로 구성하는 예외입니다.
...
각 슬라이스의 외부 진입점에 `index.ts` 같은 Public API를 두고 사용할 항목만 명시적으로 export합니다. `shared`처럼 슬라이스가 없는 레이어에서는 세그먼트별로 공개 경계를 정할 수 있습니다. 모든 내부 디렉터리에 `index.ts`를 만들라는 규칙은 아닙니다.
...
**오버엔지니어링 주의** 작은 프로젝트에서는 오히려 복잡해질 수 있습니다. 팀원 수에 정해진 최소 기준이 있는 것은 아니며, 현재 코드의 복잡도와 협업 방식, 장기 유지보수 비용을 기준으로 판단하는 게 좋아요. 모든 레이어를 처음부터 만들 필요도 없습니다.
...
- [FSD: 레이어와 import 규칙](https://feature-sliced.design/docs/reference/layers)
