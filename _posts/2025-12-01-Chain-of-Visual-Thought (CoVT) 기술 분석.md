...
Chain-of-Thought(CoT)는 LLM의 추론 능력을 개선한 기법입니다. 복잡한 문제를 단계별로 쪼개어 해결하는 방식인데, Vision-Language Model(VLM)에서는 한계가 있었습니다.
...
CoVT에서 추론 중에 생성되는 토큰은 텍스트만이 아니라 압축된 시각 잠재 표현입니다.

연구진은 시각 인식에 필요한 단서를 4가지 유형으로 나누었습니다:
...
- **Feature Alignment**: DINO처럼 표현 학습이 핵심인 모델은 특징 벡터 공간에서 유사도를 좁히는 방식으로 학습합니다.
...
기존 Latent Reasoning 모델들은 내부 동작이 블랙박스에 가까웠습니다. CoVT는 생성된 시각 토큰을 디코더와 전문가 모델의 특징에 연결해 Segmentation Mask, Depth Map 등으로 시각화할 수 있습니다. 추론에 쓰인 시각 표현을 살펴보는 단서이며, 판단의 모든 인과 과정을 설명하는 것은 아닙니다.
...
아래는 [논문 v1 표 2](https://arxiv.org/html/2511.19418v1#S3.T2)의 Qwen2.5-VL-7B와 네 유형의 시각 토큰을 사용하는 CoVT를 비교한 결과입니다. '기존 SOTA'와의 일괄 비교가 아니라 같은 기반 모델에 CoVT를 적용한 비교입니다.

| 벤치마크 | Qwen2.5-VL-7B | CoVT | 개선 (%p) |
|:---|:---:|:---:|:---:|
| CV-Bench | 74.5% | 79.8% | +5.3 |
| MMVP | 56.0% | 56.7% | +0.7 |
| RealWorldQA | 68.6% | 71.8% | +3.2 |
| CV-Bench Count | 65.0% | 66.1% | +1.1 |
| CV-Bench Depth | 72.8% | 89.2% | +16.4 |

이 설정에서는 상대적 깊이 판단의 개선이 컸고, 개수 세기의 개선은 1.1%p였습니다. 개선 폭은 과제와 시각 토큰 조합에 따라 달랐습니다.

---

## 5. 정리

CoVT는 VLM이 이미지의 공간적·기하학적 단서를 연속적인 시각 표현으로 생성하고 추론에 활용하게 만든 프레임워크입니다. 물리적 세계의 동작을 직접 시뮬레이션하는 모델은 아닙니다.

논문이 강조한 기여는 이렇습니다.
- 연속적 시각 토큰으로 텍스트 병목 완화
