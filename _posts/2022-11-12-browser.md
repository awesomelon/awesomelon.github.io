...
브라우저가 올바른 IP 주소를 받으면 서버와 connection을 빌드합니다. 웹 사이트의 HTTP 요청에는 일반적으로 TCP를 사용합니다.
...
서버에는 웹 서버(NginX, NodeJs, Apache, IIS 등)가 있습니다.
...
정적 파일은 브라우저가 캐싱해두기 때문에 나중에 해당 페이지를 방문할 때 다시 서버로부터 불러오지 않아도 됩니다.
