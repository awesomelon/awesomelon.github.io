...
**AI 기반 다중 커밋 메시지 제안**: 이 글에서 소개하는 당시 버전은 Anthropic의 Claude 3.5 모델로 커밋 메시지를 생성합니다. 변경 사항을 분석해서 여러 선택지를 보여줘요.
...
Git diff에 큰 파일이 포함되면 AI 모델의 입력 한도를 넘길 수 있습니다. 큰 파일의 내용은 빼고 파일명만 넘겨 입력 크기를 줄였어요. 다만 파일 크기와 토큰 수는 다르고 작은 파일이 많이 모여도 한도를 넘을 수 있으므로, 파일별 100KB 필터만으로 전체 요청 크기가 보장되지는 않습니다.
