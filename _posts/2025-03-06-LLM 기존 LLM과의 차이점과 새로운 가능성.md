...
기존 LLM은 트랜스포머 아키텍처 기반으로 대량의 텍스트를 학습하며 언어 이해와 생성 능력이 뛰어나다. 다만 순차적 예측이나 특정 단어에 초점을 맞춘 학습 방식이라 긴 문맥을 전체적으로 파악하는 데 한계가 있을 수 있다.
...
DLLM은 확산 모델의 원리를 텍스트에 적용한 새로운 접근이다. 텍스트 전체를 병렬 처리하고, 장기 의존성을 더 잘 다룰 수 있는 가능성이 있어 관심을 받고 있다.
